module HyperTrack
  class ApiClient
    BASE_URL = 'https://api.hypertrack.com/api'.freeze
    ...
    ACCEPTED_RESPONSE_CODES = [200, 201].freeze
    ...
    class << self
      ...
      def fetch(api_path, query_params={})
        api_path = path_with_params(api_path, query_params) if query_params.is_a?(Hash) && !query_params.keys.empty?
        api_uri = get_uri(api_path)
      ...
      def get_auth_header
        if Util.blank?(HyperTrack.secret_key)
          raise HyperTrack::InvalidAPIKey, 'HyperTrack.secret_key is invalid.'
        end
      ...
      def create_request_object(api_uri, http_method)
        if VERB_MAP[http_method].nil?
          raise HyperTrack::MethodNotAllowed,
            "Unsupported HTTP verb used - Only #{VERB_MAP.keys.map { |x| x.to_s.upcase }} allowed"
        end
